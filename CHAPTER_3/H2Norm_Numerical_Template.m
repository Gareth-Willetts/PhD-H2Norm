...
for j = 5:1:85
   for jj = 1:1
       ...
       %% Calculate H2 norm
       H2n2(counter) = (p1(1) * gamma * zk1hat)/(2*cd(1) * pkplushat * pkminushat(1));
       H2inbuilt(counter) = norm(tf(cn,cd),2)^2;
       %t(j-9*(j/10),jj) = toc;
   end
   %mean_time(j-9*(j/10)) = mean(t(j-9*(j/10),10:end));
   counter = counter + 1;
end

%%
figure
plot(5:85, (H2n2 - H2inbuilt)./H2n2)
xlabel("n"); ylabel("Proportional Difference");
title("Proportional Difference Between Numerical Method and Inbuilt Method");
set(gca, "FontSize", 20)