...
P = [1; TD2; TD3; TD4];
%P = [1; exp(-s*tau2); exp(-s*tau3); exp(-s*tau4)];
% Moments
...
s = tf('s');
f = [TF1; TF2; TF3];
[mag,phase,wout] = bode(f, {1E0 1E3}); % expects frequency in radians so 40*pi/pi
figure
subplot(2,1,1)
...
phase = squeeze(phase);
phase(1,:) = phase(1,:) - 720;
semilogx(squeeze(wout), squeeze(phase), 'LineWidth',2); xlabel("Frequency (rad/s)"); ylabel("Phase"); grid; set(gca, 'FontSize', 14);
%legend("$J_{1Z(M)}^2$", "$J_{1Z(L)}^2$", "Interpreter", "latex");
legend("Middle of train", "Leading edge of train", "Trailing edge of train", 'Location', 'southwest')

% Trapz
test = squeeze(mag).^2;
int1=trapz(squeeze(wout), test(1,:))
int2=trapz(squeeze(wout), test(2,:))
int3=trapz(squeeze(wout), test(3,:))
%sqrt(int1+int2+int3)/(sqrt(pi))
%sqrt(int1)/(sqrt(pi))
%sqrt(int2)/(sqrt(pi))
%sqrt(int3)/(sqrt(pi))
%sqrt(3.1E-5/(pi))
...
H2EstPade = sqrt((int1+int2+int3)/pi + 1.12508/pi + 0.252526/pi)
...
s = tf('s');
f = [TF1; TF2; TF3; sqrt(2.55077E11)/s^3];
[mag,phase,wout] = bode(f, {1E3 1E6}); % expects frequency in radians so 40*pi/pi
figure
%subplot(2,1,1)
loglog(squeeze(wout), squeeze(mag).^2, 'LineWidth', 2); ylabel("Magnitude"); xlabel("Frequency (rad/s)"); grid on;
set(gca, 'FontSize', 14);
%set(gca, 'YLim',[1E-3  40])
% grid
% subplot(2,1,2)
...
s = tf('s');
f = [1/(1+(s/40*pi))];
[mag,phase,wout] = bode(f, {1 1E3}); % expects frequency in radians so 40*pi/pi
figure;
%subplot(2,1,1)
loglog(squeeze(wout), squeeze(mag).^2, 'LineWidth',2); ylabel("Magnitude"); grid on; set(gca, 'FontSize', 14);  xlabel("Frequency (rad/s)");
%phase = squeeze(phase);
%phase(1,:) = phase(1,:) - 720;
%subplot(2,1,2)
%semilogx(squeeze(wout), squeeze(phase), 'LineWidth',2); ylabel("Phase"); grid; set(gca, 'FontSize', 14);
%legend("$J_{1Z(M)}^2$", "$J_{1Z(L)}^2$", "Interpreter", "latex");
%legend("Middle of train", "Leading edge of train", "Trailing edge of train", 'Location', 'southwest')

%% Bounding lower frequencies
% Bode plot for transfer functions - expand this to higher frequencies
s = tf('s');
f = [TF1; TF2; TF3; (sqrt(1.12508))*s];
[mag,phase,wout] = bode(f, {1E-3 1E0}); % expects frequency in radians so 40*pi/pi
figure
loglog(squeeze(wout), squeeze(mag).^2, 'LineWidth',2); ylabel("Magnitude"); set(gca, 'FontSize', 14); grid on;
legend("Middle of train", "Leading edge of train", "Trailing edge of train", "Upper Bound", 'Location', 'northwest')
...
% Trapz
test = squeeze(mag).^2;
expint1=trapz(squeeze(wout), test(1,:))
expint2=trapz(squeeze(wout), test(2,:))
expint3=trapz(squeeze(wout), test(3,:))
%sqrt(int1+int2+int3)/(sqrt(pi))
%sqrt(int1)/(sqrt(pi))
%sqrt(int2)/(sqrt(pi))
%sqrt(int3)/(sqrt(pi))
...
H2Exp = sqrt((expint1+expint2+expint3)/pi + 1.12508/pi + 0.252526/pi)
