...
TimeDelay = [1; TD2; TD3; TD4];
% TimeDelay = [1; 1/(1+(s*2*lbx/V)); 1/(1+(s*2*lvx/V)); 1/(1+(s*(2*(lbx+lvx))/V))];
% Obtain TF
% Plot bode diagram of exponential approximations against e^(-s*tau4) in
% frequency range 0-40*pi
TFs = [1 0; 1 lbx+lvx; 1 -lbx-lvx] * [1 0 0 0; 0 1 0 0] * (F\G) * TimeDelay * 1/(1 + (s/40*pi)) * pi*sqrt(2*Av*V); %*[d1; d2; d3; d4];
%%
% Split outputs
TF1 = TFs(1);
TF2 = TFs(2);
...
[mag,phase,wout] = bode(f, {1E-1 10}); % expects frequency in radians so 40*pi/pi
figure
subplot(2,3,1)
